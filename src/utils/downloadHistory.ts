...
export const generateAIAnalysisPrompt = (history: LearningHistory, quizSetName?: string, questions?: any[]): string => {
  ...
  // クイズセットの質問内容を整形
  let quizContentSection = '';
  if (quizSetName || questions) {
    const setName = quizSetName || 'クイズセット';
    quizContentSection = `\n[クイズセット: ${setName}]\n問題数: ${questions?.length || 'N/A'}\n`;
    
    if (questions && questions.length > 0) {
      quizContentSection += '\n問題リスト:\n';
      questions.forEach((q, index) => {
        quizContentSection += `${index + 1}. ${q.question}\n`;
        if (q.options) {
          q.options.forEach((opt: any) => {
            quizContentSection += `   ${opt.id}: ${opt.text}\n`;
          });
        }
        quizContentSection += `   (正解: ${q.correctAnswer})\n\n`;
      });
    }
  }

  return `以下のJSON形式の学習データを分析してください。ユーザーは以下のクイズセットで学習しています。${quizContentSection}
[学習データ]
