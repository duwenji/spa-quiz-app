...
「Clean Architecture」や「GitHub Copilot Skills Tutorial」など、複数のステップに分かれているクイズは、親セットとして表示されます。親セットを選ぶと各ステップの一覧が表示されます。

**受験の際は個別のステップを選択してください。**
...
3. 「**回答を確定する**」ボタンをクリックする

> **注意:** 「回答を確定する」をクリックしないと回答が記録されません。
...
同じブラウザ・同じデバイスであれば、ページを閉じても次回アクセス時に履歴が引き継がれます。
...
**回答済みの記録はローカルストレージに自動保存されます。** 同じブラウザで再度アクセスすると、以前の状態から再開できます。ただし別のブラウザや別のデバイスからは引き継げません。
