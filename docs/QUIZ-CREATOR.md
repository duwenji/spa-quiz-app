...
① 問題 JSON ファイルを作成
       ↓
② quizSets.json にメタデータを登録
       ↓
③ バリデーション実行（npm run validate:metadata && validate:quiz）
       ↓
...
| 用途 | 場所 |
|------|------|
| 問題 JSON | `public/data/<カテゴリ>/<quiz-id>.json` |
| メタデータ一覧 | `public/data/quizSets.json` |
| JSON スキーマ | `public/schemas/` |

> `public/data/` に置いたファイルはビルド後に `dist/` にコピーされ、GitHub Pages から参照されます。
...
| `npm run validate:quiz` | 問題 JSON を検証 | 開発 |
...
`public/data/quizSets.json` に、問題 JSON ファイルの情報を登録します。
...
| `difficulty` | string | ✅ | `"beginner"` / `"intermediate"` / `"advanced"` / `"beginner to intermediate"` / `"intermediate to advanced"` |
| `dataPath` | string | ✅ | `public/data/` からの相対パス（例: `"github-copilot/variables.json"`） |
...
1. `public/data/<カテゴリフォルダ>/` を作成する（フォルダがなければ）
...
`public/data/quizSets.json` を編集し、既存の配列末尾に新しいエントリを追加します。
...
# 問題 JSON の検証
npm run validate:quiz -- public/data/my-category/my-new-quiz.json
```
...
`public/data/quizSets.json` を JSON スキーマ（`public/schemas/quizset-metadata-schema.json`）で検証します。
...
```
src/data/index.ts
  └─ allQuizSets()         ← public/data/quizSets.json を読み込む
  └─ getQuizSetQuestions() ← quizSet.dataPath を fetch して問題配列を返す

public/data/
  quizSets.json            ← メタデータ一覧（全クイズ共通）
  github-copilot/
